...
interface Props {
  ...
  unreportedDamageItems?: Array<{ id: string; name: string; propertyName: string; }>;
}

export default function PendingPayablesPanel({ pendingExpenses, totalPending, onPendingClick, unreportedDamageItems }: Props) {
  const hasUnreported = unreportedDamageItems && unreportedDamageItems.length > 0;
  if (pendingExpenses.length === 0 && !hasUnreported) return null;
  return (
    <AnimatePresence>
      <motion.section
      ...
      >
        <div className="border border-yellow-200 bg-yellow-50 rounded-xl p-5">
          ...
          {hasUnreported && (
            <div className={pendingExpenses.length > 0 ? 'mt-4 border-t border-yellow-200 pt-4' : ''}>
              <p className="text-xs font-semibold text-amber-700 mb-2 flex items-center gap-1.5">
                🔧 Reparaciones pendientes de registrar
                <span className="px-1.5 py-0.5 bg-amber-100 text-amber-800 rounded-full text-[9px]">
                  {unreportedDamageItems!.length}
                </span>
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {unreportedDamageItems!.map(item => (
                  <a
                    key={item.id}
                    href={`/inventory?item_damage=${item.id}`}
                    className="bg-white rounded-lg p-3 border border-amber-200 text-left hover:border-amber-400 hover:shadow transition text-xs block"
                  >
                    <p className="font-semibold text-slate-800">{item.name}</p>
                    <p className="text-slate-400 text-[10px]">{item.propertyName}</p>
                    <p className="text-amber-700 mt-1.5 font-medium uppercase tracking-wide text-[10px]">
                      Registrar gasto de reparación →
                    </p>
                  </a>
                ))}
              </div>
